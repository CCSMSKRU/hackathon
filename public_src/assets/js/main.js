...
(function () {
	...
	MB.Core.switchPage = function (options) {
		...
		// Уничтожим предыдущий фрейм
		var frame_id = MB.Core.$pageswrap.data('frame_id');
		if (frame_id){
			var frame = MB.Frames.getById(frame_id);
			if (frame && typeof frame.remove === 'function') frame.remove({html:''});
		}
		...
		let wasTable_id = (MB.Core.$pageswrap.find('.classicTableWrap').length > 0) ? MB.Core.$pageswrap.find('.classicTableWrap').attr('data-id') : undefined;

		if(wasTable_id){
		    let was_table = MB.Tables.getTable(wasTable_id);

		    if(was_table && was_table.removeTable === 'function'){
                was_table.removeTable();
            }
        }
		...
		if (options.type) {
			if (options.type === "item" && (options.client_object || options.class)) {
				...
				return report.init();
			} else if (options.type === "frame" && (options.client_object && options.class)) {


				// debugger;

				let o = {
					container:MB.Core.$pageswrap,
					class:options.class,
					client_object:options.client_object,
					parent:null,
					ids:[],
				...
				MB.Frames.createFrame(o, (err, frame)=>{
					MB.Core.spinner.stop($('.page-content-wrapper'));
					if (err) {
						console.error(new MyError('Не удалось создать фрейм',{err:err, o:o}));
						return;
					}
	...
	MB.Core.createButtons = function (instance, selectedRows) {
		var _t = instance;
		var i = 0;
		var buttons = (_t.buttons && _t.buttons.items)? _t.buttons.items : _t.lowerButtons;
		var buttonsWrap = (_t.buttons && _t.buttons.container)? _t.buttons.container : ((_t.container.find(".lower-buttons-wrapper").last().length > 0)? _t.container.find(".lower-buttons-wrapper").last() : _t.container.find(".upper-buttons-wrapper").last());
		...
		if (buttonsWrap.length && buttons) {
			...
			for (i = 0; i < buttons.length; i++) {
				...
				var cond = button.condition? checkCondition(button.condition) : false;
				...
				if(button.operationAccess && typeof instance.access_obj === 'object'){
					let roleAccessCond = instance.access_obj[button.operationAccess];
					if (!roleAccessCond) buttonElem.addClass("hiddenByRole");
				}
...
//notification use google chrome
(function () {

    let isChrome = navigator.userAgent.indexOf('Chrome') > -1;

    if(isChrome){
        $('.use-chrome-holder').hide()
        return false;
    }



    if(Cookies.get('skip_notification_chrome_user_id' + MB.User.id) == true) {

        $('.use-chrome-holder').hide()

    }else{

        $('.use-chrome-holder').show()
        $('.use-chrome-holder').find('.hide-use-chrome').off('click').on('click', () => {
            $('.use-chrome-holder').hide()

            Cookies.set('skip_notification_chrome_user_id' + MB.User.id, true)
