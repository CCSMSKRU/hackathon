...
var classicTable = function () {
    ...
    var CTable = function (params) {
        ...
        this.changes = [];
        this.filterWhere = params.filterWhere || [];
        this.hiddenColumns = {};
        this.editedCache = [];
        ...
        $(elem).off('click').on('click', function () {
            if (!$(elem).attr('inited') && !customSelection.isSelection) {
                ...
                var selInstance = MB.Core.select3.init({
                    ...
                    id :                MB.Core.guid(),
                    parent_id:          MB.Tables.getTable(_t.id).parent_id,
                    rowId :              sRowId,
                    wrapper:            $(elem),
...
//        || ((!params.isNew && params.insertable) || (params.isNew && params.updatable))
                ...
                let val2 = params.value2 ? params.value2 : '';
                ...
                switch (params.type) {
                    ...
                    case 'like_text':
                    case 'text_like':
                        res = '<input class="ct-filter-like-text-wrapper" data-name="' + params.name + '" type="text" value="'+params.value+'">';
                        break;
                    case 'phone':
                        res = '<input type="text" class="phoneNumber" value="' + params.value + '">';
                        break;
                    case 'checkbox':
                        ...
                        if (params.isTd) {
                            ...
                            res = '<div class="ct-filter-checkbox-wrapper ' + isChecked + '" data-type="filter" data-id="' + MB.Core.guid() + '" data-name="' + params.name + '" data-value="' + params.value + '"></div>';
                        }
                        break;
                    case 'number':
                        res = '<div class="quantity"><input type="number" value="' + params.value + '"></div>';
                    ...
                    case 'daterange':
                        ...
                        res = '<div data-name="' + params.name + '" class="input-daterange input-group ct-daterange-wrapper">' +
                            '<input type="text" name="start" data-date-format="dd.mm.yyyy" value="' + params.value + '" />' +
                            '<span class="input-group-addon">-</span>' +
                            '<input type="text" name="end" data-date-format="dd.mm.yyyy" value="'+val2+'" />' +
                            '</div>';
                        break;
                    case 'datetimerange':

                        res = '<div data-name="' + params.name + '" class="input-daterange input-group ct-datetimerange-wrapper">' +
                            '<input type="text" name="start" data-date-format="dd.mm.yyyy hh:ii" value="' + params.value + '"/>' +
                            '<span class="input-group-addon">-</span>' +
                            '<input type="text" name="end" data-date-format="dd.mm.yyyy hh:ii" value="'+val2+'" />' +
                            '</div>';

                        break;
                    case 'timerange':
                        res = '<div data-name="' + params.name + '" class="input-daterange input-group ct-timerange-wrapper">' +
                            '<input type="text" name="start" value="' + params.value + '">' +
                            '<span class="input-group-addon">-</span>' +
                            '<input type="text" name="end" value="' + val2 + '">' +
                            '</div>';
                        break;
                    case 'date':
                        res = '<div class="absoluteWhiteText">' + params.value + '</div><input type="text" class="datetimepicker date" value="' + params.value + '" data-date-format="dd.mm.yyyy">';
...
// ALSURU
            ...
            _t.wrapper.find('.ct-pagination-pagesCount').html('/ ' + _t.pagesCount);
        }
    };


    CTable.prototype.mergeVisibleOnServer = function(){
        ...
        let o = {
            ...
            params: {
                user_id: MB.User.id,
                co_and_class: _t.class + '.' + _t.client_object,
                visible_data: JSON.stringify(MB.Tables.getTable(_t.id).user_settings_visible),
                noToastr: true
            }
        }
    ...
    CTable.prototype.mergeFiltersOnServer = function(){

        let _t = this;

        let o = {
            command: 'mergeFilters',
            object: 'user_setting_co_filter',
            params: {
                user_id: MB.User.id,
                co_and_class: _t.class + '.' + _t.client_object,
                filters_data: JSON.stringify(MB.Tables.getTable(_t.id).filterWhere),
                noToastr: true
            }
        }
    ...
    CTable.prototype.addWhere = function (where) {
        ...
        if(!where.isCheckbox && (!where.val1 || where.val1 === '' || where.val1 === undefined) && (!where.val2 || where.val2 === '' || where.val2 === undefined)){
            return;
        }
        ...
        let filerWhere = MB.Tables.getTable(_t.id).filterWhere;

        for (var i in filerWhere) {
            var w = filerWhere[i];

            if (w.key == where.key) {
                if ((where.val1 === '' || where.val1 === undefined) && (where.val2 === '' || where.val2 === undefined)) {
                    filerWhere.splice(i, 1);
                } else {
        ...
        if (updated == 0) {
            if (where.val1 != '' || where.val2 != '') {
                filerWhere.push(where);
            }
        }

        _t.mergeFiltersOnServer()
...
//			console.log('environment rendered');
    ...
    CTable.prototype.getValueFromUserFilterSettings = function(column_name){

        let _t  = this;

        let fw = MB.Tables.getTable(_t.id).filterWhere

        for(let i in fw){

            if(fw[i].key == column_name){
                return fw[i]
            }
        }
    ...
    CTable.prototype.render = function (callback) {
        ...
        var inlineSaveBtn
            ...
            (MB.Tables.getTable(_t.id).parent_id == undefined || MB.Tables.getTable(_t.id).parent_id == '') ?
                '<div class="ct-options-item ct-options-save"><i class="fa fa-save-"></i>&nbsp;&nbsp;Сохранить</div>' : '' : '';

        inlineSaveBtn = (MB.Tables.getTable(_t.id).virtual_where == '')? '' : '<div class="ct-options-item ct-options-save"><i class="fa fa-save-"></i>&nbsp;&nbsp;Сохранить</div>';

        var fastSearchHtml = (isFastSearch) ? '<div class="ct-fast-search-wrapper">' +
            '<input class="ct-fast-search classicTable-navigate-input" type="text" placeholder="Быстрый поиск..."/>' +
        ...
        var p_checkboxWhereHtml = (p_isCheckboxWhere) ? '<div class="ct-param-checkbox-where-wrapper"><label for="param_checkboxWhere' + _t.id + '">' + _t.profile['extra_data']['object_profile'].param_checkbox_where_title + '</label><input id="param_checkboxWhere' + _t.id + '" class="ct-param-checkbox-where" type="checkbox"' + checkboxWhereCheckedHtml + '></div>' : '';
        ...
        let notification_right_click = ''
        if (!$.cookie('skip_notification_about_right_click_on_table_user_id_' + MB.User.id) && Object.keys(this.data.data).length > 0)
            notification_right_click = '<div class="notification-right-click">' +
                '<div class="holder-notification">' +
                    '<div class="img-right-click-mouse-notification"></div>' +
                    '<span class="text-span-notification-right-click">Для открытия записи в <br> форме, кликните по строке <br> правой кнопкой мыши.</span>' +
                    '<i class="fa fa-times skip-notification_about_right_click_on_table" style="top: 10px; position: absolute; right: 10px;"></i>' +
                '</div>' +
            '</div>'

        var html =

            notification_right_click +
            '<div class="classicTableWrap" data-id="' + _t.id + '">' +
            ...
            '<div class="ct-pagination-pagesCount">/ ' + _t.pagesCount + '</div>' +
            '</div>' +
            ...
            '<div hint="Сбросить фильтры" class="ct-options-item ct-options-drop-filters"><i class="fa fa-ban-"></i></div>' +
            '<div hint="Видимость колонок" class="ct-options-item ct-options-hide-columns"><i class="fa fa-table-"></i></div>' +
            '<div hint="Дополнительные функции" class="ct-options-item ct-options-excel exclude-selection"><i class="fa fa-cogs- exclude-selection"></i></div>' +
            '<div hint="Обновить таблицу" class="ct-options-item ct-options-reload"><i class="fa fa-refresh-"></i></div>' +
            '</div>' +

            '<div class="ct-functional-fader"></div>'+

            '<div class="ct-functional-dd">' +

            '<ul class="ct-functional-list">' +
            '<li class="exportToExcel exclude-selection"><i class="fa fa-download"></i>&nbsp;Скачать отчет</li>' +
            '<li class="exportToJSON exclude-selection"><i class="fa fa-download"></i>&nbsp;Скачать JSON</li>' +
            ...
            // '<li class="ct-importFromFile exclude-selection"><i class="fa fa-download"></i>&nbsp;Импорт (Не нажимать!)</li>' +
            // '<li class="ct-configureAccess exclude-selection"><i class="fa fa-users"></i>&nbsp;Настроить доступ</li>' +
            '</ul>' +
        ...
        if (isFilters) {
            ...
            for (var f in _t.profile.data) {
                ...
                if (filterType != '') {
                    ...
                    let filterWhereData = _t.getValueFromUserFilterSettings(fName)

                    var htmlElemParams = {
                        type: filterType,
                        value: (filterWhereData)? filterWhereData.val1 : '',
                        value2: (filterWhereData)? filterWhereData.val2 : '',
                        editable: true,
                        name: fName,
                        selId: fSelId,
                        isTd: false
...
//		console.log('handlers setted', new Date(), new Date().getMilliseconds());
    ...
    CTable.prototype.getVisibleFromUserSettingsByColumnName = function(column_name){
        let _t = this;
        let userSettingsVisible = MB.Tables.getTable(_t.id).user_settings_visible;

        for(let i in userSettingsVisible){
            if(userSettingsVisible[i].key_ == column_name){
                return userSettingsVisible[i].visible
                break;
            }
        }
    ...
    CTable.prototype.renderTableHead = function (places, matrix, localStorage) {
        ...
        var result = '';
        //  fh_res += '<div>+</div>';
        places.theadRow.append('<th><span class="sharp-index">#</span></th>');
        ...
        for (var i in matrix.thead) {
            if (matrix.thead.hasOwnProperty(i)) {
                ...
                if(!_t.getVisibleFromUserSettingsByColumnName(columnName)){
                    th.addClass("hidden")
                }else{
                ...
                if (isAdd) {
                    ...
                    for (var cellName in row) {
                        if (matrix.thead.hasOwnProperty(j)) {
                            var td = $('<td data-column="'+_t.profile.data[j]["column_name"]+'"></td>');
                            if (matrix.tbody[i][j]) {
                                ...
                                if (localStorage && localStorage.columns[columnName]) {
                                    ...
                                    // if(tdW.parents('td').width <= columnWidth){
                                    //
                                    // }
    ...
    CTable.prototype.setHandlers = function () {
        ...
        _t.places.tbody.find('button.btn-secondary').each(function (index, elem) {

            if ($(elem)[0].is_iconpicker_init) return;
            $(elem).iconpicker()
            $(elem)[0].is_iconpicker_init = true
...
//			console.log($(this).text());
        ...
        _t.tds.off('click').on('click', function (e) {
            if (!customSelection.isSelection && !customSelection.isSelected) {
                ...
                if (_t.selectedRowIndex != null) {
                    primary_key = {
                        data_columns: _t.primary_keys,
                        data: _t.getPKValues(_t.selectedRowIndex)
                    };
                    if (isShift || isCtrl) {
                        if (isCtrl) {
                            markRow.click();
        ...
        _t.wrapper.find('.ct-options-filter').off('click').on('click', function () {
            ...
            if ($(innerThis).hasClass('opened')) {
                ...
                _t.wrapper.find('.ct-filter').css({display: 'none'})

                // console.log(_t.wrapper)
                // console.log(_t.wrapper.find('.ct-filter'))
            } else {
                _t.wrapper.find('.ct-filter').css({display: 'inline-block'})
        ...
        if(MB.Tables.getTable(_t.id).filterWhere.length > 0 && !_t.wrapper.find('.ct-options-filter').hasClass('opened')) _t.wrapper.find('.ct-options-filter').click()
        ...
        _t.wrapper.find('.ct-options-drop-filters').off('click').on('click', function () {
            ...
            MB.Tables.getTable(_t.id).fastSearchWhere = [];
            MB.Tables.getTable(_t.id).filterWhere = [];
        ...
        _t.wrapper.find('.ct-options-hide-columns').off('click').on('click', function (event) {
            let data = [];
            let profileData = _t.profile.data;
            let indexColumnName = "column_name";
            let indexName = "name";
            let target = $(event.target);
            let item, name, title, hidden;

            let user_settings_visible = MB.Tables.getTable(_t.id).user_settings_visible;

            function getVisibleParamFromUserSettings(name) {

                let found = false;

                for(let k in user_settings_visible){
                    if(user_settings_visible[k].key_ == name){

                        // console.log('Found', name, user_settings_visible[k].visible);

                        found = true;
                        return !user_settings_visible[k].visible
                    }
                }
            ...
            for (var i in profileData) {
                ...
                if (_t.checkColumnVisible(i)) {
                    ...
                    data.push({
                        title: title,
                        name: name,
                        selected: getVisibleParamFromUserSettings(name)
                    })
            ...
            for (var i = 0; i < listOfFilters.find('li.filterItem').length; i++) {
                ...
                switch (type) {
                    ...
                    case 'timerange':
                        ...
                        break;
                    case 'checkbox':
                        control = MB.Core.checkboxes.getItem(filter.find('.ct-filter-checkbox-wrapper').attr('data-id'));
            ...
            $('#ct-importFile-input').on('click', function(){
                ...
                il.start({
                    success: function (fileUID) {
                        ...
                        // delete il; //babel не сопринимает такую запись
                        il = undefined;
                    }
                })
            });
...
//            }
        ...
        _t.wrapper.find('.exportToExcel').off('click').on('click', function () {
            ...
            for(var k in MB.Tables.getTable(_t.id).filterWhere){
                o.params.where.push(MB.Tables.getTable(_t.id).filterWhere[k]);
            }
            ...
            var modalHtml = '<div class="row form-body" id="bootbox' + guid + '">' +
                '<div class="padder5">' +
                '<label class="wid100pr">Выгрузка ' + (tableNewInstance.profile.extra_data.object_profile.name_ru || 'записей') + ' с (номер записи):' +
                '<input type="text" class="form-control start_no" value="' + start_no + '"/>' +
                '</label><br><br>' +
                '<label class="wid100pr">Количество записей:' +
                ...
                buttons: {
                    download_all: {
                        ...
                        callback: function(){
                            ...
                            var info = toastr.info('Идет процесс выгрузки...');
        ...
        _t.wrapper.find('.exportToJSON').off('click').on('click', function () {
            var tableNewInstance = MB.Tables.getTable(_t.id);
            // var o = {
            //     client_object: tableNewInstance.profile.extra_data.object_profile.client_object,
            //     name: tableNewInstance.profile.extra_data.object_profile.client_object_name,
                    ...
                    success: {
                        ...
                        callback: function () {
                            ...
                            socketQuery(o, function(r){
                                ...
                                btn[0].click();
                                // toastr[r.toastr.type](r.toastr.message + '</br><a id="' + id + '"  style="color:blue;" target="_blank"  href="' + path + filename + '"></a>'  );
...
//					console.log(MB.Core.validator.time($(this).val()));
        ...
        _t.wrapper.find('.skip-notification_about_right_click_on_table').on('click', function() {
            _t.wrapper.find('.notification-right-click').hide()

            Cookies.set('name', 'value');

            $.cookie('skip_notification_about_right_click_on_table_user_id_' + MB.User.id, true)
        })
    ...
    CTable.prototype.setHighlights = function(){
        ...
        for(let i in _t.data.data){
            let r = _t.data.data[i];


            for(let k in _t.highlight_row){

                let h = _t.highlight_row[k];
                ...
                switch (h.type){
                    ...
                    case '=':
                        ...
                        if(r[h.column] == h.value){

                            highlight_arr.push({id: r.id, color: h.color, idx: i});
        ...
        // console.log('hightlight_arr', highlight_arr);


        for(let l in highlight_arr){

            _t.places.tbody.find('tr').eq(highlight_arr[l].idx).addClass('highlight_' + highlight_arr[l].color);
    ...
    CTable.prototype.hideColumn = function (columnName, hidden) {
        let _t = this;
        let thead = _t.places.theads[columnName];

        let columnIndex = thead.index();
        let tds = _t.places.tds;

        let tds2 = _t.wrapper.find('tbody td[data-column="'+columnName+'"]')
        ...
        if (hidden) {
            ...
            tds2.addClass("hidden");
        } else {

            thead.removeClass("hidden");
            tds2.removeClass("hidden");
        }
        ...
        for(let k in userSettingsVisible){

            if(userSettingsVisible[k].key_ == columnName){
                userSettingsVisible[k].visible = !hidden
                found = true
            }
        }
        ...
        if(!found){
            ...
            userSettingsVisible[Object.keys(userSettingsVisible).length] = {
                key_: columnName,
                visible: !hidden,
            }
    ...
    CTable.prototype.hideTable = function () {
        ...
        console.log('user_settings_visible', user_settings_visible);


        let columns_from_profile = [];
        let hiddenCount = 0;

        for(let i in _t.profile.data){
            if(_t.profile.data[i].visible){
                columns_from_profile.push(_t.profile.data[i])
            }
        }

        for(let k in user_settings_visible){
            for(let j in columns_from_profile){
                if(user_settings_visible[k].key_ == columns_from_profile[j].column_name && user_settings_visible[k].visible == false){
                    hiddenCount++
                }
            }
        }

        // console.log('hiddenCount', hiddenCount, ':', hiddenCount, columns_from_profile.length, hide);

        if(hiddenCount == columns_from_profile.length){
            hide = true;
        }
