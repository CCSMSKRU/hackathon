var select3init = function () {
	...
	var Select3 = function (params) {
		var id = params.id || MB.Core.guid();
		this.id =                       id;
		this.select3List = 				params.select3List;
		...
		this.depend_value =            params.depend_value || undefined;
	...
	Select3List.prototype.clear = function (){
		var _t = this;
		for (var i in _t.items) {
			var item = _t.items[i];
		}
...
//        }
	...
	Select3.prototype.init = function () {
		...
		var template = '<div class="select3-wrapper" id="{{id}}">' +
			...
			'{{#data}}' +
			'<li data-value="{{id}}" data-sysname="{{sysname}}">{{name}}</li>' +
			'{{/data}}' +
			'</ul>' +
			'</div>' +
			'</div>' +
		...
		_t.rowTemplate = '{{#list}}<li data-value="{{id}}" data-sysname="{{sysname}}">{{#multiSelect}}<i class="fa fa-circle-o"></i>{{/multiSelect}}<span>{{name}}</span></li>{{/list}}';

		var data = {
			id: _t.id,
			value_id: _t.value.id,
			value_name: _t.value.name,
			sysname: _t.value.sysname,
			data: _t.data,
	...
	Select3.prototype.setValue = function () {
		...
		var was = {
			...
			sysname: _t.output.attr('data-sysname')
		};
		var now = {
			id: _t.value.id,
			name: _t.value.name,
		...
		_t.wrapper.attr('data-sysname', _t.value.sysname);
	...
	Select3.prototype.getData = function (callback) {
		...
		if (_t.type == 'inited') {
			...
			if (_t.additional_params_for_get_data) {
				o.params = {
					...o.params,
					additional_params: {
						..._t.additional_params_for_get_data
					}
				}
			}
	...
	Select3.prototype.destroy = function(){
		var _t = this;
		for (var i in _t.select3List.items) {
			if (_t.select3List.items[i].id === _t.id) _t.select3List.items.splice(i,1);
		}
		_t.wrapper.remove();
	...
	Select3.prototype.addParamsGetForSelect = function(obj) {
		this.additional_params_for_get_data = obj
	};
		...
		if(_t.type == 'inited'){
			...
			for (var i = _t.lastIndex; i < _t.data.length; i++) {
				...
				if (_t.isFilter) {
					...
					sysname = item['sysname'];//(_t.fromServerNameString == '') ? item[1] : (item[names.indexOf(_t.fromServerNameString.toUpperCase())]) ? item[names.indexOf(_t.fromServerNameString.toUpperCase())] : item[names.indexOf('CLIENT_VALUES')];
				}

				data.list.push({id: id, name: name, sysname:sysname, multiSelect: _t.multiSelect});
			}
		}
...
//		_t.resultsScroll.scrollTop(0);
	...
	var initFunction = function (params, callback) {
		params.select3List = select3List;
