...
var modalWindow = function(){
    ...
    var mWItem = function(params){
        ...
        this.title =            params.name || params.title || 'Создание';
        ...
        this.hideReloadButton =   params.hideReloadButton || false;
        ...
        this.config_access =    params.config_access === undefined ?  true : params.config_access;
    };
...
//        console.log(tryLineWidth / (tryLength+1));
    ...
    mWItem.prototype.reInitActionBtns = function(obj){
        var _t = this;
        obj = obj || {};
        _t.hideReloadButton = (typeof obj.hideReloadButton !== "undefined")? obj.hideReloadButton : _t.hideReloadButton;
        _t.hideFullscreen = (typeof obj.hideFullscreen !== "undefined")? obj.hideFullscreen : _t.hideFullscreen;
        var instance_to_reload = obj.instance_to_reload || _t.formInstance;

        var html =
            (_t.hideReloadButton ? '' : '<div class="mw-reload-from"><i class="fa fa-refresh"></i></div>') +
            '<div class="mw-collapse">_</div>'
            + (_t.hideFullscreen ? '' : '<div class="mw-fullscreen"><i class="fsc-icon"></i><i class="nfsc-icon"></i></div>')
                + '<div class="mw-close"><i class="fa fa-times"></i></div>';
        _t.wrapper.find('.mw-actionBtns').html(html);

        // setHandlers
        _t.wrapper.find('.mw-reload-from').off('click').on('click', function(){
            instance_to_reload.reload();
        });
        _t.wrapper.find('.mw-fullscreen').off('mousedown').on('mousedown', function(e){
            if(e.button == 0){
        ...
        _t.wrapper.find('.mw-collapse').off('click').on('click', function(){
            _t.collapse();
        });

        _t.wrapper.find('.mw-close').off('click').on('click', function(e){
    ...
    mWItem.prototype.render = function(callback){
        var _t = this;
        var isSaveBtn = (_t.hideSaveButton)? '' : '<div class="mw-save-form disabled">Сохранить <span class="ctrl_s">(Ctrl+S)</span></div>';
        ...
        var template = '<div class="mw-wrap ' + _t.client_object + ' activeW ' + _t.className + '" id="mw-{wrapId}" style="top:' + _t.top + 'px; left: ' + _t.left + 'px; height: ' + getHeightStr + '; width:' + _t.width + 'px;">' +
...
//                console.log('_t.top: ', _t.top, '_t.left: ', _t.left);
    ...
    mWItem.prototype.close = function(){
        ...
        if (_t.formInstance && typeof _t.formInstance.remove === 'function') typeof _t.formInstance.remove();
        _t.footerButtonIsRemove = true;
