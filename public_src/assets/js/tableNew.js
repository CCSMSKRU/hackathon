...
(function () {
	...
	MB.TableN = function (params) {
        ...
        this.updateIntervalTime = params.updateIntervalTime || 60000;
        this.updateInterval;
        this.filterWhere = [];
        this.user_settings_visible = [];
	...
	MB.TableN.prototype.create = function (wrapper, callback) {
		...
		// _t.getUserSettings(function(){
            _t.getProfile(_t.client_object, function () {
                ...
                _t.getUserSettings(function(){
                    _t.fillFilterWhereByUserSettings();
                    _t.getData(function () {
...
//		console.log('RESULT WHERE: ', result);
    ...
    MB.TableN.prototype.getUserSettings = function(cb){
        ...
        socketQuery(o, function(res){
            if(!res.code){
                ...
                _t.user_settings_filter = res.user_settings_filter;
                _t.user_settings_visible = res.user_settings_visible;
        ...
        //         },{
        //             key: 'co_and_class',
        //             val1: _t.class + '.' + _t.client_object,
        //         }]
        //     }
        // }
        //
        ...
        //     if(!res.code){
        //         _t.user_settings_filter = res.data;
        //         cb()
        //     }
        // })
        //
        ...
        //
        //     if(!res.code){
        //         _t.user_settings_visible = Object.keys(res.data).length > 0 ? res.data : [];
        //         // cb()
        //     }
        // })
    ...
    MB.TableN.prototype.fillFilterWhereByUserSettings = function(){

        let _t = this;

        for(let i in _t.user_settings_filter){
            let filter = _t.user_settings_filter[i]
            ...
            _t.filterWhere.push(f_toAdd)
        }

        console.log('be_filterWhere', _t.filterWhere);
...
// 		var enableWhere = (_t.ct_instance) ? (_t.ct_instance.filterWhere.length > 0) : false;
		var enableWhere = _t.filterWhere.length > 0;
		...
		if (_t.profile['extra_data']['object_profile']['checkbox_where'] != '') {
			...
			if (chkbx.length && chkbx.is(':checked') || !chkbx.length && (_t.profile['extra_data']['object_profile']['checkbox_where_default_enabled'])) {
                ...
                for(var i in ch_where_data){
					...
					cw.val1 = (typeof cw.val1 === 'string')? cw.val1.replace(/\$NOW_DATE/ig, function (c) {
		...
		bs.remove.off('click').on('click', function () {
			...
			if (!removeButton.hasClass("disabled")) {
				...
				if (_t.remove_processing) {
					...
					if (diff < 15000){
						toastr.info('Удаление... Ожидайте результата. ' + diff);
						return;
					}
				}
...
//        });
    ...
    /**
     * updates table data once per N minutes
     * @type {CTables}
     */
    MB.TableN.prototype.updateDataClientCron = function(){

        let _t = this;
        let interval_time = _t.updateIntervalTime;
        let updateClasses = ['request_work'];

        if(_t.updateInterval) clearInterval(_t.updateInterval);

        _t.updateInterval = setInterval(function(){

            if ($('body').find('.ct-environment-wrapper[data-id="'+_t.id+'"]').length > 0){

                if(updateClasses.indexOf(_t.class) > -1){
                    _t.reload();
