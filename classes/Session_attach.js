...
Model.prototype.createSessid = function (obj, cb) {
    if (arguments.length == 1) {
        cb = arguments[0];
        obj = {};
    }
    ...
    async.series({
        createSession: cb => {
            ...
            _t.api(o,(err, res) => {
                if (err) return cb(new MyError('Не удалось создать сессия прикрепления файлов',{o : o, err : err}));
                cb(null, res);
            })
        }
    }, (err, res) => {
        if (err) cb(err)
        if (res.createSession) cb(null, res.createSession)
    });
