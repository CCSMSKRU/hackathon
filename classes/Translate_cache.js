    ...
    async.series({
        getAll:function(cb){
            ...
            _t.get(params, function (err, res) {
                if (err) return cb(new MyError('Не удалось получить translation_cache',{params : params, err : err})); // Could not get
                for (var i in res) {
                    var lang = res[i].lang;
                    if (!global.translations) global.translations = {};
                    if (!global.translations[lang]) global.translations[lang] = {};
                    global.translations[lang][res[i].alias] = res[i].text_corrected || res[i].text_;
                }
                cb(null);
            });
        }
        ...
        cb(null, new UserOk('noToastr',{translations:global.translations}));
    });
};
